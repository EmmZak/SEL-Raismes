    ...
    <template v-slot:top>
      ...
      >
        ...
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="text-h10">Vous êtes sûr de vouloir supprimer l'admin ?</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
      ...mapActions(["fetchAdmins", "removeAdmin", "saveAdmin"]),
      save () {
        this.saveAdmin(this.editedItem)
        this.close()
        this.notify("Admin créé")
      },
      // UPDATE
      editItem (item) {
      ...
      deleteItemConfirm () {
        ...
        this.removeAdmin(this.itemToDelete)
        this.itemToDelete = {}
        this.closeDelete()
        this.notify("Admin supprimé")
      },
      close() {
        this.dialog = false
